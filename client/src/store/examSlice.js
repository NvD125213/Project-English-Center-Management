...
export const getDetailExam = createAsyncThunk('exam/getDetailExam', async ({ examId }) => {
  const response = await api.get(`/exam/getbyID/${examId}`);
  return response.data;
});
...
const examSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(getDetailExam.fulfilled, (state, action) => {
        state.loading = false;
        state.questionByPart = action.payload.questionGroups
      })
      .addCase(getDetailExam.rejected, (state) => {
        state.loading = false;
      });
  },
});
